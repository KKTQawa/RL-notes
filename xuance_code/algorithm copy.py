...
class RoboticWarehouseEnv:
    """Wrapper for rware environment."""
    def __init__(self, env_id: str, seed: int = 0):
        import rware
        # Parse env_id like "rware-tiny-2ag-v2"
        self.env = simple_spread_v3.parallel_env(N=3, local_ratio=0.5, max_cycles=25, continuous_actions=False)
        # rware import might fail; fallback to MPE
        self.env.reset(seed=seed)
        self.agents = self.env.possible_agents
        self.agent_keys = self.agents
        self.n_agents = len(self.agents)
