...
class MixingQNetwork(nn.Module):
    def __init__(self, obs_dim, n_agents, n_actions, dim_state,
        ...
        self.eval_mixer = QMIXMixer(dim_state, mixer_hidden, hyper_hidden, n_agents, device)
...
class MARLReplayBuffer:
    def __init__(self, n_envs, buffer_size, batch_size, n_agents, obs_dim, act_dim, state_dim):
        self.n_envs = n_envs
    ...
    def clear(self):
        ...
        self.agent_mask = np.ones(shp + (self.n_agents,), dtype=np.bool_)
    ...
    def sample(self, batch_size=None):
        ...
        return {
            'obs': self.obs[env_choices, step_choices],
            'actions': self.actions[env_choices, step_choices],
            'rewards': self.rewards[env_choices, step_choices],
            'terminals': self.terminals[env_choices, step_choices],
            'obs_next': self.obs_next[env_choices, step_choices],
            'state': self.state[env_choices, step_choices],
            'state_next': self.state_next[env_choices, step_choices],
            'batch_size': bs,
        }
...
class QMIXAgent:
    def __init__(self, config):
        ...
        self.n_envs = config.parallels
        self.obs_dim = config.obs_dim
        self.act_dim = config.act_dim
        self.state_dim = config.state_dim

        self.policy = MixingQNetwork(
        ...
        total_iters = (config.running_steps - config.start_training) // (config.training_frequency * self.n_envs)
        ...
        self.delta_egreedy = (config.start_greedy - config.end_greedy) / max(1, config.decay_step_greedy / self.n_envs)
...
def build_envs(config):
    return [RoboticWarehouseEnv(config) for _ in range(config.parallels)]
...
def train(config):
    ...
    for step in pbar:
        ...
        obs_next_list, rewards_list, dones_list = [], [], []
        for i in range(n_envs):
            o, r, d, tr, _ = envs[i].step(actions[i])
            obs_next_list.append(o)
            rewards_list.append(r)
            dones_list.append(d or tr)
            episode_rewards[i] += r.mean()
            episode_steps[i] += 1

        obs_next = np.stack(obs_next_list, axis=0)
        rewards = np.stack(rewards_list, axis=0)
        terminals = np.stack(dones_list, axis=0)
        state_next = get_global_state(obs_next)

        # store per env
        for i in range(n_envs):
            agent.store_experience(
                obs[i], actions[i], rewards[i], np.full(n_agents, terminals[i]),
                obs_next[i], state[i], state_next[i]
            )
        ...
        # handle episode end
        for i in range(n_envs):
            if terminals[i] or episode_steps[i] >= config.max_episode_steps:
                train_episode += 1
                if config.use_tqdm:
                    pbar.set_description(f"Ep {train_episode} R={episode_rewards[i]:.2f} eps={agent.e_greedy:.3f}")
                episode_rewards[i] = 0
                episode_steps[i] = 0
                obs[i] = envs[i].reset()
                state[i] = get_global_state(obs[i:i+1])[0]

        if step % log_interval == 0 and total_steps >= config.start_training:
            avg_r = np.mean([np.mean(episode_rewards) for _ in range(n_envs)])  # rough
            print(f"Step {step}/{config.running_steps} | Episodes: {train_episode} | Eps: {agent.e_greedy:.4f} | "
                  f"Loss: {loss:.4f} | Q_tot: {q:.4f}")

        if step % config.eval_interval == 0 and step > 0:
            eval_reward = evaluate(config, agent)
            print(f"Evaluation at step {step}: avg_reward = {eval_reward:.4f}")
            agent.save_model(f"{config.model_dir}/step_{step}.pth")

    for env in envs:
        env.close()
    agent.save_model(f"{config.model_dir}/final.pth")
