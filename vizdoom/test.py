...
def plot_curves(reward_history, loss_history, env_id):
    ...
    plt.title(f"DQN-{env_id}-version1 Episode Loss")
    plt.xlabel("Episode")
    plt.ylabel("Loss")

    plt.tight_layout()
    plt.savefig(f"DQN-{env_id}--version1-training.png")
    plt.show()
...
def test(env_id="VizdoomBasic-v1", model=None, trial=3, device="cuda" if torch.cuda.is_available() else "cpu"):
    ...
    for ep in range(trial):
        ...
        print(f"[TEST] {ep}: reward={total_reward:.2f}")
        os.system("pause")
    env.close()
...
if __name__ == "__main__":
    ...
    model, reward_history, loss_history = train("VizdoomHealthGathering-v1")
    plot_curves(reward_history, loss_history, "VizdoomHealthGathering-v1")
    os.system("pause")
    test("VizdoomHealthGathering-v1",model=model,trial=3)