...
$u=\mu+\epsilon\sigma ,\epsilon \sim \mathcal{N}(0, I)$
...
$\log\pi(a)=\log p(u)-\log(1-\tanh^2(u))= \log p(u)-2*\log 2-u-\operatorname{softplus}(-2u)$
...
$L_{alpha}=\mathbb{E}(-\log\alpha(\log\pi(a|s)+H_0)),取H_0\approx action dim$

## 深度推导

>总目标最大化

$J(\pi)=\mathbb{E}_t[L_t]=\mathbb{E}_t[\gamma^t(r_t+\alpha \mathcal{H})]$

同时保证

$\mathcal{H} \ge H_0,\mathcal{H}=-\mathbb{E}_{\pi} \log\pi(a|s)$


$\therefore J(\pi)=\mathbb{E}_t[L_t]=\mathbb{E}_t[\gamma^t(r_t-\alpha \log\pi(a|s))]$

### actor_loss来源

$\because 最优策略\pi^*(a|s)\propto \exp\left(\frac{1}{\alpha}Q(s,a)\right)$
...
&\stackrel{\text{忽略无关项}}{==} \mathbb{E}_{a \sim \pi_\phi}\left[\log \pi_\phi(a|s) - \frac{1}{\alpha}Q(s,a)\right]
\end{aligned}
$$
